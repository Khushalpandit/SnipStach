...
// Create folder
router.post('/', createFolderValidation, validateRequest, async (req, res, next) => {
    try {
...
// Update folder
router.patch('/:id', updateFolderValidation, validateRequest, async (req, res, next) => {
    try {
        const { id } = req.params;
        const update = req.body;
        const folder = await Folder.findOneAndUpdate({ _id: id, userId: req.user.id }, { $set: update }, { new: true });
        if (!folder) {
...
router.delete('/:id', async (req, res, next) => {
    try {
        ...
        await Snippet.updateMany({ folderId: id, userId: req.user.id }, { $unset: { folderId: 1 } });
        // Delete folder
