import { AutoTag, Language } from '@snipstash/shared';

const tagPatterns: Record<AutoTag, RegExp[]> = {
  'loop': [
    /\b(for|while)\s*\(/i,
...
const languageSpecificPatterns: Partial<Record<Language, Record<AutoTag, RegExp[]>>> = {
  'javascript': {
    'async': [
      /\bnew\s+Promise\b/i,
      /\b\.then\(/i,
...
export const generateAutoTags = (code: string, language: Language): AutoTag[] => {
  const tags = new Set<AutoTag>();

  // Check common patterns
  Object.entries(tagPatterns).forEach(([tag, patterns]) => {
    if (patterns.some(pattern => pattern.test(code))) {
      tags.add(tag as AutoTag);
    }
  });
